...
void InjectingInstrumentationFunction(const BasicBlock::iterator &I,
  ...
  //          {Addr->getType(), Type::getInt32Ty(CTX), Type::getInt32Ty(CTX)},
  //          false);
  FunctionCallee InstrumentationFunction =
      M.getFunction("_Z13numCacheLinesPvjj");
  Builder.CreateCall(
      InstrumentationFunction,
      {Addr, Builder.getInt32(LocationCounterSourceMap[SourceAndAddrSpaceInfo]),
       typeSizeVal});
}
