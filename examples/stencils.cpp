...
int main(int argc, char **argv) {
    ...
    int align = 1;
    // Offset pointers such that thread accesses are cache-line aligned
    int use_offset = 0;
    ...
    if (argc > 6) 
        align = atoi(argv[6]);
    if (argc > 7) 
        use_offset = atoi(argv[7]);
    
    printf("Settings: nx = %d ny = %d nz = %d nt = %d nw = %d align = %d use_offset = %d \n",
            nx, ny, nz, nt, nw, align, use_offset);
